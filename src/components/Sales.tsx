      ...
      <div>
        {filteredSales.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#94A3B8' }}>
            ...
            <p style={{ fontSize: '16px', fontWeight: '600', color: '#475569', marginTop: '12px' }}>
              {sales.length === 0 ? 'Продаж пока нет' : 'Продаж не найдено'}
            </p>
            <p style={{ fontSize: '14px' }}>
              {sales.length === 0 ? 'Оформите первую продажу' : 'Попробуйте изменить фильтр или оформите новую продажу'}
            </p>
          </div>
        ) : (
          <div style={{
          ...
          {[...filteredSales]
            .sort((a, b) => {
              ...
              return (
                order[a.status?.toLowerCase()] ?? 1
              ) - (
                order[b.status?.toLowerCase()] ?? 1
              );
            })
            .map((sale) => {
            ...
            const saleImage = getSaleProductImage(sale);

            const sizeMatch =
              sale.productName?.match(
                /\((\d+\.?\d*)\)/
              );
            const sizeDisplay =
